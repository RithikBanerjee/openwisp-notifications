...
class AbstractNotification(UUIDModel, BaseNotifcation):
    ...
    @classmethod
    def invalidate_related_object_cache(cls, obj_id):
        """
        Invalidate related objects cache.
        """
    ...
    def _related_object(self, field):
        ...
        if not obj_id:
            return
        cache_key = self._cache_key(obj_id)
        obj = cache.get(cache_key)
        if not obj:
            obj = getattr(self, f'_{field}')
            cache.set(
                cache_key, obj, timeout=app_settings.OPENWISP_NOTIFICATION_CACHE_TIMEOUT
            )
        return obj
